...
class BME680(BME680Data):
    ...
    def __init__(self, i2c_addr=constants.I2C_ADDR_PRIMARY, i2c_device=None):
        ...
        if COMMUNICATION_MODE==1: # use I2C interface
            import smbus
            ...
            wiringpi.pinMode(6, 1)  # cs pin           

        try:
            if COMMUNICATION_MODE==1:
                print("i2c read chip id")
            else:
                print("spi read chip id")
            self.chip_id = self._get_regs(constants.CHIP_ID_ADDR, 1) #0x61
            print("Chip_ID:",hex(self.chip_id))
            if self.chip_id != constants.CHIP_ID:
                raise RuntimeError('BME680 Not Found. Invalid CHIP ID: 0x{0:02x}'.format(self.chip_id))
        except OSError:
            raise RuntimeError("Unable to identify BME680 ")
    ...
    def soft_reset(self):
        ...
        if COMMUNICATION_MODE==0:  # use SPI
            self._get_mem_page()
    ...
    def _get_mem_page(self):
        wiringpi.digitalWrite(6,0) # cs =0
        self.spi.writebytes([MEM_PAGE_ADDR|SPI_RD_MSK])
        #self.spi.write(bytearray([MEM_PAGE_ADDR|SPI_RD_MSK]))
        reg = int.from_bytes(self.spi.readbytes(1),'little')&MEM_PAGE_MSK
        wiringpi.digitalWrite(6,1) # cs =1
        return reg
    
    def _set_mem_page(self,reg_addr):
        if reg_addr > 0x7F:
            mem_page = MEM_PAGE1
        else:
            mem_page = MEM_PAGE0
        if mem_page != self.mem_page:
            ...
            wiringpi.digitalWrite(6,0) # cs =0
            self.spi.writebytes([MEM_PAGE_ADDR|SPI_RD_MSK])
            reg = int.from_bytes(self.spi.readbytes(1),'little')
            wiringpi.digitalWrite(6,1) # cs =1
            reg = reg&(~MEM_PAGE_MSK)
            reg = reg|(self.mem_page&MEM_PAGE_MSK)
            wiringpi.digitalWrite(6,0) # cs =0
            self.spi.writebytes([MEM_PAGE_ADDR&SPI_WR_MSK,reg])
            #self.spi.write(bytearray([reg]))
            wiringpi.digitalWrite(6,1) # cs =1
            
    def _set_regs(self, register, value):
        """Set one or more registers."""
        if COMMUNICATION_MODE==1:  # use I2C
            if isinstance(value, int):
                self._i2c.write_byte_data(self.i2c_addr, register, value)
            else:
                self._i2c.write_i2c_block_data(self.i2c_addr, register, value)
        else:  # use SPI
            self._set_mem_page(register)
            #self.cs.value(0)
            wiringpi.digitalWrite(6,0) # cs =0
            self.spi.writebytes([register&SPI_WR_MSK,value])
    ...
    def _get_regs(self, register, length):
        ...
        if COMMUNICATION_MODE==1:  # use I2C
            ...
            self._set_mem_page(register)
            register = register|SPI_RD_MSK
            #self.cs.value(0)
            wiringpi.digitalWrite(6,0) # cs =0
            self.spi.writebytes([register])
            temp = self.spi.readbytes(length)
            #self.cs.value(1)
            wiringpi.digitalWrite(6,1) # cs =1
            if length == 1:
                return temp[0]
